...
shell_wizards_menu(){ while :; do c=$(ui_menu 'Shell configuration wizards' 'Configure shells with selectable options.' bash 'Complete Bash wizard' zsh 'Complete Zsh wizard' fish 'Complete Fish wizard' ash 'Ash wizard' ksh 'Ksh wizard' profile 'POSIX login profile wizard' inputrc 'Readline/inputrc wizard' starship 'Starship prompt wizard' plugins 'Shell plugin manager') || { _menu_rc=$?; [ "$_menu_rc" -eq "${UI_MENU_BACK_RC:-90}" ] && return 0; return "$_menu_rc"; }; case $c in bash) bash_full_wizard;; zsh) zsh_full_wizard;; fish) fish_full_wizard;; ash) ash_wizard "$CURRENT_HOME/.ashrc";; ksh) ksh_full_wizard;; profile) profile_wizard "$CURRENT_HOME/.profile";; inputrc) inputrc_wizard "$CURRENT_HOME/.inputrc";; starship) starship_wizard "$CURRENT_HOME/.config/starship.toml";; plugins) plugin_manager_menu shell;; esac; done; }
...
editor_wizards_menu(){ while :; do c=$(ui_menu 'Editor configuration wizards' 'Configure editors through selectable options.' nano 'Nano option wizard' vim 'Vim option wizard' nvim 'Neovim Lua wizard' micro 'Micro settings wizard' helix 'Helix TOML wizard' emacs 'Emacs Lisp wizard' plugins 'Editor plugin manager') || { _menu_rc=$?; [ "$_menu_rc" -eq "${UI_MENU_BACK_RC:-90}" ] && return 0; return "$_menu_rc"; }; case $c in nano) nano_wizard "$CURRENT_HOME/.nanorc";; vim) vim_wizard "$CURRENT_HOME/.vimrc";; nvim) nvim_full_wizard;; micro) micro_full_wizard;; helix) helix_full_wizard;; emacs) emacs_full_wizard;; plugins) plugin_manager_menu editor;; esac; done; }
...
terminal_wizards_menu(){ while :; do c=$(ui_menu 'Terminal configuration wizards' 'Configure terminal behavior without raw editing.' env 'Terminal environment wizard' tmux 'Tmux wizard' screen 'GNU Screen wizard' inputrc 'Readline/inputrc wizard' less 'Less pager wizard' starship 'Starship prompt wizard' test 'Terminal capability test' plugins 'Terminal plugin manager') || { _menu_rc=$?; [ "$_menu_rc" -eq "${UI_MENU_BACK_RC:-90}" ] && return 0; return "$_menu_rc"; }; case $c in env) terminal_full_wizard;; tmux) tmux_wizard "$CURRENT_HOME/.tmux.conf";; screen) screen_wizard;; inputrc) inputrc_wizard "$CURRENT_HOME/.inputrc";; less) less_wizard;; starship) starship_wizard "$CURRENT_HOME/.config/starship.toml";; test) terminal_test;; plugins) plugin_manager_menu terminal;; esac; done; }
