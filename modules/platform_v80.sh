...
platform_v80_menu(){ while :; do c=$(ui_menu 'v8 RootFS Platform' "Active: $(active_rootfs)" projects 'RootFS Projects' recipes 'Build Recipes' workspace 'Multi-RootFS Workspace' diff 'Advanced Diff Engine' monitor 'Resource and workflow monitoring' remote 'Remote RootFS guidance' docs 'Documentation Generator' visualizer 'Workflow Visualizer') || { _menu_rc=$?; [ "$_menu_rc" -eq "${UI_MENU_BACK_RC:-90}" ] && return 0; return "$_menu_rc"; }; case $c in projects) rootfs_projects_menu;; recipes) build_recipes_menu;; workspace) multi_rootfs_menu;; diff) rootfs_deep_diff;; monitor) platform_monitor_menu;; remote) remote_rootfs_menu;; docs) documentation_generator_menu;; visualizer) workflow_visualizer_menu;; esac; done; }
