...
# systui — Enhanced Devuan GNU+Linux Provisioning
#
# Features:
...
provision_devuan_enhanced() {
    ...
    if [ -n "$apt_mirror" ] && [ "$apt_mirror" != "default" ]; then
        run_cmd "Configuring APT mirror" sh -c "
            sed -i \"s|deb.devuan.org|$apt_mirror|g\" /etc/apt/sources.list
        "
    fi
    ...
    # ========== PACKAGE PROFILES ==========
    local pkgs_base="bash bash-completion coreutils findutils grep sed gawk diffutils util-linux bsdmainutils procps passwd file less openssh-client ca-certificates openssl man-db manpages curl wget rsync tzdata locales"
    
    local pkgs_standard="$pkgs_base openssh-server sudo build-essential python3 python3-pip python3-venv vim nano tmux sysstat htop ncdu lsof pv tree fzf ripgrep fd-find bat eza jq yq nmap tar unzip zip p7zip-full bzip2 gzip zstd xz-utils git chrony cron logrotate syslog-ng"
    
    local pkgs_full="$pkgs_standard neovim emacs perl ruby ruby-dev golang rustc cargo nodejs npm lazygit tig mercurial subversion meson ninja-build scons bazel lldb valgrind gdb strace ltrace wireshark tcpdump socat netcat-openbsd iperf3 mtr dnsutils iproute2 bridge-utils wireguard openvpn openconnect sqlite3 redis-tools postgresql-client mysql-client"
    ...
    local pkgs_server="$pkgs_standard openssh-server syslog-ng fail2ban aide logrotate rsync lvm2 mdadm cryptsetup btrfs-progs nginx postgresql redis-server bind haproxy keepalived"
    
    local pkgs_container="$pkgs_base openssh-server openssh-client sudo docker.io docker-compose podman buildah skopeo runc containerd cri-o kubectl helm terraform ansible python3 python3-pip"
    ...
    # Fix binary name differences in Devuan
...
FAIL2BAN
    ...
    if [ "$enable_apparmor" = 1 ]; then
        run_cmd "Enabling AppArmor" sh -c "
            ...
            update-rc.d apparmor enable 2>/dev/null || true
        "
    fi
   ...
   Devuan GNU+Linux  .  iSH-AOK  (enhanced, init freedom)
   -------------------------------------------------------
   Profile  : PROFILE_NAME
   services :  service <name> {start|stop|status}
   on boot  :  update-rc.d <name> {enable|disable}
   status   :  service <name> status   logs : tail -f /var/log/syslog
...
NICETIES
    ...
    # ========== SERVICE MANAGEMENT ==========
    local services="ssh syslog-ng cron chrony"
    [ "$enable_fail2ban" = 1 ] && services="$services fail2ban"
    [ "$enable_apparmor" = 1 ] && services="$services apparmor"
    
    for s in $services; do
        service "$s" enable >/dev/null 2>&1 || update-rc.d "$s" enable >/dev/null 2>&1 || true
        service "$s" restart >/dev/null 2>&1 || true
    done
    
    # ========== SUMMARY ==========
    tui_msg "Devuan Enhanced Provisioning Complete" \
        "Devuan GNU+Linux has been provisioned successfully!\n\n
Profile: $profile
Shell: $shell
