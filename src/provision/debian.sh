...
provision_debian_impl() {
    ...
    local pkgs="bash bash-completion cmake coreutils findutils grep sed gawk diffutils util-linux bsdextrautils procps passwd adduser file less locales openssh-client openssh-server sudo rsyslog iputils-ping chrony cron logrotate tzdata ca-certificates openssl man-db manpages curl wget rsync bind9-dnsutils iproute2 git strace build-essential gdb python3 python3-pip python3-venv vim neovim nano tmux sysstat htop btop ncdu lsof pv tree mc fzf ripgrep fd-find bat eza jq most w3m lynx nmap socat netcat-openbsd mtr tar unzip zip p7zip-full bzip2 gzip zstd xz-utils fastfetch figlet ncurses-bin ncurses-term"
    
    if apt-get install -y --no-install-recommends $pkgs; then
        log "Packages installed"
    else
        log "Batch install failed; retrying package-by-package"
        for p in $pkgs; do
...
export -f provision_debian_impl provision_debian
