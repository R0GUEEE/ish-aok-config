...
menu_brew_install() {
    ...
    c=$(tui_menu "Install Homebrew" "Choose installation method:" \
        root   "Root-compatible installer (iSH-AOK / Debian arm64, LD_PRELOAD shim)" \
        back   "Back") || return 0
    case "$c" in
        root)
            if [ "$(id -u)" -ne 0 ]; then
            ...
            tmp=$(mktemp /tmp/brew-root-install-XXXXXX.sh)
            chmod 0700 "$tmp"
            cat > "$tmp" <<'__BREW_INSTALLER__'
#!/usr/bin/env bash
...
cat > /etc/profile.d/homebrew.sh <<EOF_PROFILE
export HOMEBREW_PREFIX="$BREW_PREFIX"
export HOMEBREW_CELLAR="$BREW_PREFIX/Cellar"
export HOMEBREW_REPOSITORY="$BREW_REPOSITORY"
export HOMEBREW_NO_ANALYTICS=1
...
The actual Homebrew executable is:
  $REAL_BREW
EOF_DONE
__BREW_INSTALLER__
            run_cmd "Install Homebrew (root-compatible)" bash "$tmp"
            rm -f "$tmp" ;;
        back|"") return 0 ;;
    esac
}
